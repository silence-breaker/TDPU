...
> 硬件约束: PSDDR **1 GB** (PS 侧, Linux 系统占用部分), PLDDR **4 GB** (PL 侧, 64-bit DDR3)

| 区域名称 | 存储位置 | 大小 (Size) | 访问方式 | 说明 |
| :--- | :--- | :--- | :--- | :--- |
| Model Weights | PLDDR | 600 MB | PL 直接访问; PS 通过驱动写入 | 存放预处理后的 BitNet 2-bit 压缩权重 (`.bin`) |
| KV Cache | PLDDR | 2 GB | PL 直接读写; PS 通过 CDMA 搬运 | 存放 Attention 的 Key/Value 矩阵 (FP16/INT8) |
| Output Buffer | PLDDR | 64 MB | PL 直接写入; PS 通过 CDMA 回读 | FPGA 计算结果 (INT32) 回写区域 |
| Activation Buffer | PSDDR | 128 MB | PS 直接读写; PL 通过 HP 口读取 | 存放待发送给 FPGA 的 INT8 激活值 |

PLDDR 使用量: 600 MB + 2 GB + 64 MB ≈ **2.66 GB** / 4 GB (66.5%)
PSDDR 使用量: 128 MB / 1 GB (12.8%, 其余留给 Linux 系统)

> 说明: KV Cache 和 Output Buffer 放在 PLDDR 而非 PSDDR，因为 PSDDR 仅 1GB 且需预留给 Linux 系统。PL 模块可直接高带宽访问 PLDDR；PS 侧需要读取结果时通过 CDMA 搬运到 PSDDR。PSDDR 地址由 Linux 内核分配 (通过 CMA 或 `dma_alloc_coherent`)，PLDDR 地址由 PL 端 DDR 控制器决定。
